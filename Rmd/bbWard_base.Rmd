...
```{r}
mapPlot("Connections.receiving.superfast.speeds......30.Mb.s.")
```


```{r}
mapPlot("Availability.of.Superfast.broadband.." )
```

```{r}
mapPlot("Ave.Download.Speed.Mb.s")
```
```{r}
mapPlot("Connections.receiving.speeds.under.2.Mb.s")
```
```{r}
mapPlot("Connections.receiving.speeds.under.10.Mb.s")
```
```{r}
mapPlot( "Connections.unable.to.receive.10.Mb.s")
```
```{r}
mapPlot("Average.speed.for.basic.broadband.lines..Mb.s.")
```
```{r}
mapPlot("Average.speed.for.superfast.lines..Mb.s.")
```
```{r}
mapPlot("Avaiability.of.Ultrafast.Broadband...300.Mb.s...")
```
```{r}
mapPlot("Estimated.data.completenessa")
```
```{r}
mapPlot("Rank..Ave.download.speed..1.best..8.668.worst.")
```
```{r}
mapPlot("Rank..Connections.over.30.Mb.s....1.best.")
```
```{r}
mapPlot("Rank..Connections.under.10.Mb.s..1.worst.")
```
