...
rr=regionsquarematrix2015 %>% gather(from, value, 3:ncol(.))
...
s=gvisSankey(rr[!is.na(rr$value),c('Region','from','value')], to="Region", from="from", weight="value")
#And render it
plot(s)
```
...
colnames(rr) = c("sourceName","target","source","value")
...
rrd=data.frame(rid=c(unlist(regionsquarematrix2015$Region),unlist(paste0(regionsquarematrix2015$Region,'_'))))
rrd['num']=0:(nrow(rrd)-1)
rrd['name']=c(unlist(regionsquarematrix2015$DESTINATION),unlist(paste0(regionsquarematrix2015$DESTINATION,'_')))
